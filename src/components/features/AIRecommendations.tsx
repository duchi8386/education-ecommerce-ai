...
import { Sparkles, RefreshCw, TrendingUp, Zap, Brain, Target } from 'lucide-react';
  ...
  const handleGetRecommendations = async () => {
    ...
    try {
      ...
      if (recommendationType === 'personalized') {
        ...
        const viewedProducts = mockProducts.filter(p => userHistory.includes(p.id));
        const categories = [...new Set(viewedProducts.map(p => p.category))];
        
        if (categories.length > 0) {
          newRecommendations = mockProducts
            .filter(p => categories.includes(p.category) && !userHistory.includes(p.id))
            .slice(0, 3);
        } else {
          newRecommendations = mockProducts
        ...
        if (recentlyViewed) {
          const recentProduct = mockProducts.find(p => p.id === recentlyViewed.productId);
          if (recentProduct) {
            newRecommendations = mockProducts
              .filter(p => p.category === recentProduct.category && p.id !== recentProduct.id)
              .slice(0, 3);
          }
        }
    ...
    <section className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 lg:p-8">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between mb-8">
        <div className="mb-4 lg:mb-0">
          <div className="flex items-center space-x-2 mb-2">
            ...
            <h2 className="text-2xl font-bold text-gray-900">Gợi ý thông minh từ AI</h2>
          </div>
          <p className="text-gray-600">
            Hệ thống AI phân tích sở thích và đề xuất khóa học phù hợp nhất cho bạn
          </p>
        </div>
              ...
              <div className="mt-3 p-3 bg-gradient-to-r from-purple-50 to-blue-50 rounded-lg">
                <div className="flex items-center space-x-2">
                  ...
                  <span className="text-sm text-gray-700">
                    {recommendationType === 'personalized' && 'Phù hợp với sở thích của bạn'}
                    {recommendationType === 'trending' && `${product.students.toLocaleString()} học viên đang theo học`}
                    {recommendationType === 'similar' && 'Tương tự khóa học bạn đã xem'}
                  </span>
                </div>
              </div>
            </div>
      ...
      <div className="mt-8 p-4 bg-gradient-to-r from-blue-50 to-purple-50 rounded-xl">
        <div className="flex items-start space-x-3">
          ...
          <div>
            <h3 className="font-semibold text-gray-900 mb-1">Thông tin từ AI</h3>
            <p className="text-sm text-gray-700">
              Dựa trên phân tích {state.viewHistory.length} khóa học bạn đã xem và {state.favoriteProducts.length} khóa học yêu thích, 
              chúng tôi nhận thấy bạn quan tâm đến các lĩnh vực công nghệ và phát triển kỹ năng cá nhân.
            </p>
          </div>
        </div>
      </div>
