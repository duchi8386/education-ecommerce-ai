...
export class RecommendationService {
  ...
  static analyzeUserBehavior(
    ...
    [...viewedProducts, ...likedProducts].forEach((productId) => {
      ...
      if (product) {
        ...
        categoryPreferences[product.category] =
          (categoryPreferences[product.category] || 0) + weight;
        levelPreferences[product.level] =
          (levelPreferences[product.level] || 0) + weight;
        instructorPreferences[product.instructor] =
          (instructorPreferences[product.instructor] || 0) + weight;
    ...
    if (likedProductsPrices.length > 0) {
      const avgPrice =
        likedProductsPrices.reduce((sum, price) => sum + price, 0) /
        likedProductsPrices.length;
      priceRange = {
   ...
   */
  static calculateProductSimilarity(
    product1: Product,
    product2: Product
  ): number {
    let similarity = 0;
  ...
  static generateSmartRecommendations(
    ...
    candidateProducts.forEach((product) => {
      ...
      const categoryScore =
        (userBehavior.categoryPreferences[product.category] || 0) / 10;
      score += categoryScore * 0.3;

      // 2. Điểm dựa trên level yêu thích
      const levelScore =
        (userBehavior.levelPreferences[product.level] || 0) / 10;
      score += levelScore * 0.2;

      // 3. Điểm dựa trên instructor
      const instructorScore =
        (userBehavior.instructorPreferences[product.instructor] || 0) / 10;
      ...
      if (
        product.price >= userBehavior.priceRange.min &&
      ...
      if (categoryScore > 0.5) {
        reason = `Bạn đã quan tâm đến ${
          userBehavior.categoryPreferences[product.category]
        } khóa học ${product.category}`;
        ...
        const topTags = product.tags.filter(
          (tag) => userBehavior.tagPreferences[tag] > 0
        );
      ...
      if (score > 0.1) {
        // Chỉ gợi ý sản phẩm có điểm đủ cao
